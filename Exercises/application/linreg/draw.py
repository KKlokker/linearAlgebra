...
def make_fig(ax, fit_func, model, x, y):
    global a,b
    ax.plot(x,y,'o')
    ...
    try:
        ...
        print(e, "'None' is returned when the method is not implemented.")
    return -1
...
ax = plt.subplot(141)
f=lambda xx: a*xx + b
_lin = make_fig(ax, linear_model, f, x,y)


ax = plt.subplot(142)
f = lambda xx: a*np.exp(b*xx)
_exp = make_fig(ax, exponential_model, f, x, y)


ax = plt.subplot(143)
f = lambda xx: a*(xx**b)
_pow = make_fig(ax, power_model, f, x, y)


ax = plt.subplot(144)
f = lambda xx: a+b*np.log(xx)
_log = make_fig(ax, logarithmic_model, f, x, y)
