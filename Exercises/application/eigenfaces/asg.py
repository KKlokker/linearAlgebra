...
def projectAndBackProject(dataImg, mean, eigenVectors):
    ...
    340
    """
    project = cv2.PCAProject(dataImg,mean.reshape(dataImg.shape),eigenVectors)
    return cv2.PCABackProject(project,mean.reshape(dataImg.shape),eigenVectors).reshape(dataImg.shape)
...
def indexDistancePairsSortedByDistance(data, r, mean, eigenVectors):
    ...
    pairs = []
    for i in r:
        distance = distanceImages(projectAndBackProject(data[i,:], mean, eigenVectors), data[i,:])
        pairs.append((i,distance))
