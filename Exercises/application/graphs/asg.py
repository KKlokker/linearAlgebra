...
def allPermutationMatrices(n):
    ...
    if n <= 0:
        raise ValueError()
    result = []
    for p in itertools.permutations(np.identity(n)):
        result.append(np.array(p,np.int64))
    return result
...
def isIsomorphicUsingP(A, B, P):
    ...
    if A.shape != B.shape or B.shape != P.shape:
        raise ValueError()
    if not isPermutationMatrix(P):
        raise TypeError()
    return np.array_equal(A,P@(P@B).T)
...
def numIsomorphisms(A, B):
    ...
    0
    """
    Ps = allPermutationMatrices(len(A))
    counter = 0
    for P in Ps:
