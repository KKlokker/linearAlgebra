...
def add(u, v):
    ...
    sumList = list(map(lambda x,y: x+y,u.store,v.store))
    return Vec(sumList)
...
def dot(u, v):
    ...
    """
    assert u.size == v.size
    mulList = list(map(lambda x,y: x*y,u.store,v.store))
    return sum(mulList)
...
def scalar_mul(v, alpha):
    ...
    True
    """
    scalList = list(map(lambda x: x*alpha, v.store))
    return Vec(scalList)
