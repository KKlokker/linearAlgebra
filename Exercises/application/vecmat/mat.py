...
def add(A, B):
    ...
    """
    assert A.size == B.size
    sumList = list(map(lambda Arow,Brow: list(map(lambda x,y: x+y,Arow,Brow)),A.store, B.store))
    return Mat(sumList)
...
def scalar_mul(M, x):
    ...
    True
    """
    scalarList = list(map(lambda Arow: list(map(lambda c: c*x,Arow)),M.store))
    return Mat(scalarList)
...
def transpose(M):
    ...
    True
    """
    return Mat(list(map(list,zip(*M.store))))
...
def vector_matrix_mul(v, M):
    ...
    return matrix_vector_mul(transpose(M.copy()), v)
...
def matrix_vector_mul(M, v):
    ...
    mulList = list(map(lambda row: sum(list(map(lambda column, vC: column * vC, row,v.store))), M.store))
    return Vec(mulList)
...
def matrix_matrix_mul(A, B):
    ...
    mulList = list(map(lambda rowA: list(map(lambda rowB: sum(list(map(lambda a, b: a * b, rowA, rowB))), transpose(B.copy()).store)), A.store))
    return Mat(mulList)
